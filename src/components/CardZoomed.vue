...
<template>
  <Teleport to="body">
    ...
    >
      <div ref="content" class="content" :style="{ '--card-color': card.color }">
        <button class="close-btn" @click="handleClose">{{ t('zoom.close') }}</button>
        <span class="content-icon">{{ card.icon }}</span>
        <h2 class="content-title">{{ t(`card.${card.id}.title`) }}</h2>
        <p class="content-desc">{{ t(`card.${card.id}.desc`) }}</p>
      </div>
    </div>
  </Teleport>
</template>
